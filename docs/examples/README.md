...
- [`nginx-restart`](nginx-restart/docker-compose.yml): allow logs and restarts for nginx.
...
The container-specific examples prefer `dpp.rule.self.*` labels. Those labels are
